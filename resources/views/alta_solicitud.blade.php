...
<div id="alta_solicitud" >
    ...
    <form class="mb-5 mt-2" id="alumno" v-on:submit.prevent="guardar()">
        ...
        <div class="form-group">
            ...
            <select class="form-control form-control-md" name="perfil" id="perfil" v-model="solicitud.perfil" @change="getCategorias()" required>
                <option value="" selected="selected" disabled>Selecciona</option>
                    <option :value="perfil.id" v-for="perfil in Perfiles" v-if="perfil.id != 1">@{{perfil.nombre}}</option>
            </select>
        </div>
        <div class="row">
            <div class="form-group col-6" v-if="solicitud.perfil != ''">
                <label for="">Categoría del problema</label>
                <select class="form-control form-control-md" name="categoria" id="categoria" v-model="solicitud.categoria" @change="getSubcategorias()" required>
                    <option value="" selected="selected" disabled>Selecciona</option>
            ...
            <div class="form-group col-6" v-if="solicitud.categoria != ''">
                ...
                <select class="form-control form-control-md" name="subcategoria" id="subcategoria" v-model="solicitud.subcategoria" @change="mostrar_inputs()" required>
                    <option value="" selected="selected" disabled>Selecciona</option>
                    <option :value="subcategoria.id" v-for="subcategoria in Subcategorias">@{{subcategoria.nombre}}</option>
                </select>
            </div>
            <div class="form-group col-6" v-if="Preguntas" v-for="pregunta in Preguntas">
                <label for="">@{{pregunta.texto}}</label>
                <select class="form-control form-control-md" name="subcategoria" id="subcategoria" required>
                    <option value="" selected="selected" disabled>Selecciona</option>
                    <option :value="subcategoria.id" v-for="opcion in pregunta.opciones">@{{opcion.texto}}</option>
                </select>
            </div>
            ...
            <div class="form-group col-md-6" v-if="show_inputs">
                ...
                <input class="form-control form-control"  v-model="solicitud.correo_contacto"  required data-inputmask="'alias': 'email' " placeholder="Correo de contacto">
            </div>
            ...
            <div class="input-group" v-if="show_inputs">
                <input type="text" class="form-control form-control-md file-upload-info" disabled="" placeholder="Seleccionar archivos de ayuda">
            ...
            <div class="form-group col-md-12 mt-3" v-if="show_inputs">
                ...
                <textarea class="form-control form-control-md text-area" name="descripcion_solicitud" id="" rows="10" v-model="solicitud.descripcion" placeholder="Escribe aquí la descripción del problema" required></textarea>
            </div>
